...
本节讲解使用 Node 来做一个脚手架，便于快速开发项目。我们开发的是脚手架，而非项目，目前本人只有一个基础 [Koa 脚手架](https://github.com/johanazhu/koa-basic) ，后续写到 React、webpack 时，会搭建属于自己的一套 H5 端的开发模板。本文以实现最小脚手架为出发点展开写作，后续也会再次基础上添砖加瓦
...
看 [ create-react-app](https://github.com/facebook/create-react-app)、 [vue-cli](https://github.com/vuejs/vue-cli)、[@tarojs/cli](https://github.com/NervJS/taro/tree/next/packages/taro-cli) 的各自的仓库，我们能出一些共同点，例如多套模板、友好的交互、优美的UI等等。我们这里以 taro 为例，先用用，看看，再仿着做一个
...
一：使用 `shelljs` 常有报错，暂时解决不了，所以用 `download-git-repo` 这种方式
...
Github已发布：[https://github.com/johanazhu/azhu-cli](https://github.com/johanazhu/azhu-cli)
