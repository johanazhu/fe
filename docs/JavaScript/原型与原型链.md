...
**在JS中每个函数都有一个 prototype 属性，它实际指向的是一个 prototype 对象**，比如有一个函数 Foo，我们来看看 Foo.prototype 这个对象里到底有什么：
...
> 每一个 JavaScript 对象（除了 null）都具有一个属性，叫 __proto__，这个属性会指向该对象的原型
