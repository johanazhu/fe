...
我们在 `foo` 内部加了一句 `obj.x = 2`，计算前的 `counter.x` 是1，但是计算以后 `counter.x` 是 2。`foo` 函数的执行对外部的 `counter` 产生了影响，他又副作用，因为它修改了外部传进来的对象，它不纯了。
...
- [浅析JavaScript函数式编程](https://juejin.cn/post/6940442700889980965)
