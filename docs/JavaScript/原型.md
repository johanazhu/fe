...
2）后者只需要一个对象，它将作为我穿件的新对象的原型 const B = Object.create(A)。即以 A 为原型创建对象

class 的继承，继承的是行为和结构，但没有继承数据。

而基于 prototype 的继承，可以继承数据、结构和行为三者

`__proto__`是对象所独有的，并且`__proto__`是**一个对象指向另一个对象**，也就是他的原型对象

`prototype`是函数所独有的**。**它的作用就是包含可以给特定类型的所有实例提供共享的属性和方法。

任何函数在创建时，都会默认给改函数添加 `prototype` 属性
...
![原型](D:\code\project\elaine\fe\docs\.vuepress\public\images\JavaScript\原型.png)
...
![原型默认继承](D:\code\project\elaine\fe\docs\.vuepress\public\images\JavaScript\原型默认继承.png)
...
![function的定义](D:\code\project\elaine\fe\docs\.vuepress\public\images\JavaScript\function的定义.png)
...
![prototype与proto](D:\code\project\elaine\fe\docs\.vuepress\public\images\JavaScript\prototype与proto.png)
...
![new之后的user](..\.vuepress\public\images\JavaScript\new之后的user.png)

我们在构造函数中定义的属性 this.firstname,this.lastname 以及 动作 this.callMyName 。在 new User 时，继承了User（构造函数）中的所有prototype（即前面说的 User.props）对象
