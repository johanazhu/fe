...
1）前者需要两个对象，将A设置为B 的原型。Object.setPropertyOf(A, B)

2）后者只需要一个对象，它将作为我穿件的新对象的原型 const B = Object.create(A)。即以A为原型创建对象
...
const obj_a = { a: 1 }
const obj_b = { b: 1 }
Object.setPrototypeOf(obj_a, obj_b)
console.log(obj_b)
```

![显示原型继承](D:\code\project\elaine\fe\docs\.vuepress\public\images\JavaScript\显示原型继承.png)
...
![Object.create显示原型继承](D:\code\project\elaine\fe\docs\.vuepress\public\images\JavaScript\Object.create显示原型继承.png)

除了 Object.setPrototypeOf 方法以外，还有另一种途径。即是通过 Object.create 方法，直接继承另一个对象。

Object.setPropertyOf 和 Object.create 的差别在于：

1）Object.setPropertyOf ，给我两个对象，我把其中一个设置为另一个原型。
...
![setPrototypeOf显式绑定原型](D:\code\project\elaine\fe\docs\.vuepress\public\images\JavaScript\setPrototypeOf显式绑定原型.png)
...
如果将 constructor 函数的 prototype 改名为 properties。问题可能少很多。或者进一步简写成props，理解上将更加简单。不就是 new Constructor 时，自带了它的 Constructor.props 对象嘛
...
const user2 = new User('elaine', 'ma')
console.log(user1)
console.log(user2)
```

![new之后的user](D:\code\project\elaine\fe\docs\.vuepress\public\images\JavaScript\new之后的user.png)

我们在构造函数中定义的属性 this.firstname,this.lastname 以及 动作 this.callMyName 。在 new User 时，自带了 User.prototype（即前面说的User.props）对象
...
做法很简单，将 constructor 设置为空函数（let Noop = function() {}）,相当于没有了属性初始化的过程，将 Noop的原型绑定赋值给传来的 proto（关联原型），最后返回基于空函数的实例化对象（return new Noop）
...
const createInstance = (Constructor, ...args) => {
    let instance = Object.create(Consturctor.prototype) 
    // 创建一个基于Constructor的原型的实例
    Constructor.call(instance, ...args)
    // 通过call，将Constructor中的this 指向 instance
    return instance;
    // 返回实例
}
```
...
![基于createInstance的user对象](D:\code\project\elaine\fe\docs\.vuepress\public\images\JavaScript\基于createInstance的user对象.png)
...
作为对象的prototype，不仅可以被继承，还能被当作值来传递，它跟其他普通对象，并没有区别。
...
如上图所示，不管我们通过 class fields 语法，还是在 constructor 里面声明数据。最后，它们都会出现在实例对象上，而非原型对象上。

![prototype屏蔽继承数据能力迎合class](D:\code\project\elaine\fe\docs\.vuepress\public\images\JavaScript\prototype屏蔽继承数据能力迎合class.png)
...
![prototype继承数据](D:\code\project\elaine\fe\docs\.vuepress\public\images\JavaScript\prototype继承数据.png)
