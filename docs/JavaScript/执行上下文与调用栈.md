...
总结的很完整，按照 `选新不选旧` 原则，本文应该以 ES2022 为切入点展开，最次也要 ES2018，但主流的解释执行上下文都以 ES3 为例，权衡之后，笔者将以 ES5 为基础撰写执行上下文，并在后续补充说明 ES3 中的执行上下文
...
JavaScript 引擎室按照可执行代码来执行代码的，每次执行步骤如下：
