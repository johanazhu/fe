...
可以看出，Object 只能拷贝第一层对象，如果再往深一层拷贝，就成引用了。
...
综上分析，JavaScript 的浅拷贝有 4 种，其中三种是针对 数组的浅拷贝，只有 Object.assign() 是针对对象
...
深拷贝 JSON.pase(JSON.stringify()),浅拷贝+递归
...
那么有没有一种方法，只有当属性修改以后才对这部分数据做深拷贝，又能解决 JSON.parse(JSON.stringify(a))的局限，答案是 Proxy，具体细节请看这篇文章，本人暂时还么有能力打败这关

https://juejin.im/post/5df7175fe51d45582512962c

总结的话，深拷贝是前端面试中必考的一项，它问你怎么手写，你若是只写了 JSON.parse(JSON.stringify(source))肯定是不合格的。有这个问题，就是因为它能考到很多知识，

比如说 数据类型的判断，for 循环，原型链，递归以及 hash 表（key-value 形式存储数据）等等
