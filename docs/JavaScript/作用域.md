...
注意哦，你代码写在哪里，你的作用域就定位在哪里，在例子中，foo 函数和 bar 函数是同级（同一层面），它们内部的变量互不影响，这就是词法作用域
...
### 动态作用域（假设 JavaScript 中有动态作用域，实际上是没有的）
...
在 ES6 之前我们是没有块级作用域的，ES6 中的 `let关键字` , `const关键字` 能锁住作用域
...
原因很简单，因为 let 自带块级作用域，详细情况我将在 ES6 系列中阐述，这里不做过多介绍。只需要知道有 let 和 const 的地方，它定义的变量 i 就被包裹在块级作用域中（域有绝对领域，变量自成一方）

这样我们就又有种方法解决 for 循环中 setTimeout 自变量改变的问题了。这里希望你能明白一个知识点。ES6 之前 JavaScript 是没有块级作用域概念的，只能通过立即执行函数来做出块级作用域的效果，而 ES6 之后，let，const 关键字能起到块级作用域的效果
...
本想在作用域陷阱中一起讲完，但是这块知识过于重要，单独拿出来讲会比较好
...
变量提升，函数提升。后者覆盖前者，function 的权重大于 var
