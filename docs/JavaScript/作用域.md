...
笔者这里有个回调函数，当你明白这个例子，你对作用域的理解就已登堂入室：
...
原因很简单，因为 let 自带块级作用域，详细情况笔者会在讲 ES6 文章中进行说明，这里不做过多介绍。只需要知道有 let 和 const 的地方，它定义的变量 i 就被包裹在块级作用域中（域有绝对领域，变量自成一体）
...
每一个引用的库引入就执行，变量存在于所在作用域中，库于库之间因为函数独立，所以命名方法互不影响，就不会勿扰到全局

换个方式理解：立即执行函数 = 匿名函数 + 立即执行
