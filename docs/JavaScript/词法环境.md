...
在介绍 `词法环境` 前，我们先看下在V8里 JavaScript 的编译执行过程，大致分为三个阶段
...
当你看到 `var a = 1;` 时，可能会认为这是一个声明。但 JavaScript 实际上会将其看成两个声明：`var a = undefined;` 和 `a = 2;` 。第一个定义声明在编译阶段进行。第二个赋值声明会**被留在原地**等待执行阶段。  
