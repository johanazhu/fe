...
> React 最核心的  Virtual DOM 、diff算法，打从最开始的 virtual-dom 库开始，人们就发现更快的 diff 优化方案，原来官方做的东西不是最优的（fb方面其实更关注于其平台覆盖度，让它可以在各种平台上运行，而不是DOM环境 ）。于是大家在virtual-dom上面的改动才会更来劲。diff算法基本上把字符串算法那套挪过来了，比如最短编辑长度距离，最长上升序列。。。
>
