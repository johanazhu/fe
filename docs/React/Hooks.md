...
在使用 useState 时，就会蹦出一个常见的面试题，即函数式组件能捕获渲染时的值，可移步[函数式组件与类组件有何不同](./函数式组件与类组件有何不同.md) 中查看

第二个会遇到的面试题时：如果useState 中的值的数据类型为 Object 时，修改其中的一个值，是否会影响到其他值的渲染

例如我

```jsx
const Home = () => {
	const [person, setPerson] = useState({name: 'johan', age: 28})
    return (
        <div>
