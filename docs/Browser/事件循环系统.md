...
之前讲 [浏览器架构](./浏览器架构) 时，我们说到现代浏览器采用多进程架构，有 浏览器主进程、渲染进程、插件进程、网络进程、GPU 进程等组成。又在 [渲染进程中的线程](./渲染进程中的线程.md) 中讲到，渲染进程由 JS 引擎线程、GUI 渲染线程、事件触发线程、定时触发线程、异步 HTTP 请求线程等。而且 GUI 渲染线程与 JS 引擎线程是相斥的
